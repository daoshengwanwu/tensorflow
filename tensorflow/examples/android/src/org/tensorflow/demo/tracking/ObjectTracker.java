...
public class ObjectTracker {
    ...
    public class TrackedObject {
        ...
        TrackedObject(final RectF position, final long timestamp, final byte[] data, String title) {
            isDead = false;
            this.title = title;
    ...
    public synchronized TrackedObject trackObject(
            final RectF position, final long timestamp, final byte[] frameData, String title) {
        if (downsampledTimestamp != timestamp) {
            ObjectTracker.downsampleImageNative(
                    frameWidth, frameHeight, rowStride, frameData, DOWNSAMPLE_FACTOR, downsampledFrame);
            downsampledTimestamp = timestamp;
        }
        return new TrackedObject(position, timestamp, downsampledFrame, title);
    }

    public synchronized TrackedObject trackObject(final RectF position, final byte[] frameData, String title) {
        return new TrackedObject(position, lastTimestamp, frameData, title);
    }

    public int getCurrentPeopleNum() {
        int num = 0;
        for (TrackedObject to : trackedObjects.values()) {
            if ("person".equals(to.title)) {
                num++;
            }
        }
        return num;
